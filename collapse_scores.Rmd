...
Last, load in counts of subs from GISAID alignment (up to something like 16 million sequences) and compare frequency of deletion mutant to ACE2 effect
...
p1 <- ggplot(data=gisaid_dels, aes(y=log10(frequency), x=delta_bind))+
  ...
  geom_text_repel(aes(label=ifelse((log10(frequency) > -6),as.character(site),''), color = ifelse(site %in% sig_mAb_sites, "yes", "no")),size=3, show.legend=FALSE)+
  scale_color_manual(
